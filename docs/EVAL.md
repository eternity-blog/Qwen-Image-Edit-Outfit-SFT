...
`eval_viton_holdout.py` 会输出这个值（**n=200 留出集上是 20.87**，stdev 12.26、中位数 17.46）。
...
实测（**n=200**，seed 0 / 40 步 / 768×1024）：

| 模型 | MAD vs person | MAD vs teacher | 判读 |
|---|---|---|---|
| *基线 teacher vs person* | *20.87* | *0* | 刻度 |
| base | 40.14 | 38.95 | 改动量近 2 倍且比交白卷更远 → 大改且改错 |
| lora_v2_lr1e-4 | 21.58 | 9.10 | 改动量对，且明显优于交白卷 |
| **full_sft_b1** | **21.40** | **8.76** | 改动量几乎正中刻度 → 改得又对又适量 |

训练后三个模型的改动量 21.40–21.59 与刻度 20.87 相差不到 4%，
而距 teacher 只有 8.76–9.10（远低于 20.87 的及格线）——
**说明它们不是靠「少改动」压低指标，而是改对了地方**。

> **样本量口径**：本页数字一律取自 **n=200** 留出集。
> 早期曾用 6 样本，其刻度是 18.64、base 是 35.65/28.94——
> 那 6 条对未训练底座偏易，导致训练收益被低估约三分之一。
> **不要混用两个样本量的数字**（详见 [EVAL_RESULTS_20260815.md](EVAL_RESULTS_20260815.md) 第 1.4 节）。
