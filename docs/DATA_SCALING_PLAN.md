...
### 方案 5 — 多参考图（解锁线上真实入参）※ 2026-08-16 提升为 P0

线上一次最多传 3 张商品参考图（不同视角）。当前训练全是单张，模型没有多图融合能力。

**2026-08-16 的对照实验把它从「锦上添花」变成了「最大单点故障」**（见
[EVAL_RESULTS 第 2.1 节](EVAL_RESULTS_20260815.md)）：同一个模型、同一套 v2 模板，
仅把商品参考图从 3 张降到 1 张，未训练的 base 就从「整幕重绘」变成「局部编辑」
（MAD vs 源帧 72.77 → 32.49）。也就是说，**输入图数量与训练不一致本身就足以触发失效**，
其破坏力大于 prompt 长度差异。

VITON 无同款多视图，故此项曾长期 deferred；可行解：
1. 用同一张服装图的裁切 / 翻转 / 局部放大构造 2–3 个"视角"（成本最低，但不是真多视图）
2. DressCode 或电商商品图（主图 + 细节图 + 平铺图）—— 真实多视图
3. 训练时随机化 `n_product_refs ∈ {1,2,3}`，让模型学会"多图指向同一件商品"
...
2026-08-16 的对照实验重排了优先级：case02 崩溃的主因是**参考图数量**而非 prompt 长度。

| 阶段 | 内容 | 成本 | 预期效果 |
|---|---|---|---|
| **P0** | **方案 5 多参考**：训练时随机化 `n_product_refs ∈ {1,2,3}` | 转换脚本改动 + 一次训练 | 直击已证实的最大单点故障 |
| P1 | 方案 4 真实帧 + GPT teacher（先几百条打通链路） | 数据收集为主 | 补目标域，并监督字幕与颜色保真 |
| P2 | 方案 1 prompt 表层增广 | 分钟级转换 | 仍值得做，但已知不是 case02 主因 |
| P3 | 方案 2 扩 pair（batch2 已完成）+ 方案 3 DressCode | teacher GPU 时间 | 提升保真度上限、补类目 |
