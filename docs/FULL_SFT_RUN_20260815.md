...
8×H100 80GB + ZeRO-3 + bf16,把 Qwen-Image-Edit-2511 的 DiT(20.43B 参数)在 11415 条换装 pair 上做了 1 epoch 全参 SFT,**2h22m 跑完,1427/1427 步无报错,产出 40.86GB 的 DiT-only checkpoint**。loss **窗口均值** 0.0409 → 0.0311(降 23.7%),无 OOM,显存峰值 74.8GB(91.7%),8 卡负载极度均衡。

> 用窗口均值而非首末单步:扩散训练每步随机 timestep 与噪声,末 100 步单步 stdev 就有 0.0249、极差 0.1034,**单步噪声远大于整体趋势**,引用首步 0.029 / 末步 0.024 会得出站不住的结论。详见第 7 节。
