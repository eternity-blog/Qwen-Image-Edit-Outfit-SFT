...
> 起因：2026-08-15 已完成全参 SFT（LR=1e-5, 1 epoch）。本文设计**同一方案内的单变量超参扫描**——一次只动一个旋钮，其余钉死在基线，用同一套 held-out 评测判断每个配置是否优于基线。
>
> **⚠️ 2026-08-16 更新：本文写作时的两个前提已被后续实验推翻，读前必看。**
>
> 1. **「LoRA 尚未评测、与全参不可比较」已不成立。** 同数据同步数的对照实验已完成
>    （`train_lora_v2_multigpu.sh`，1427 步与全参一致），并做了 4 个 LR 点的扫描。
>    结论：**域内**配对检验下 r16 LoRA 与全参**无法区分**；**业务域** 3 张参考图时
>    只有全参不崩。见 [EVAL_RESULTS_20260815.md](EVAL_RESULTS_20260815.md) 第 1.1、2.2 节。
>    因此 LoRA 的 LR 扫描（本文 Tier 1）**已经做完**，不必重跑。
> 2. **业务域失效的主因已定位，不是超参问题。** 对照实验显示 case02 崩溃的主因是
>    **输入参考图数量与训练不一致**（3 张 vs 训练的 1 张）——同一模型仅改此项，
>    未训练的 base 都能从整幕重绘恢复为局部编辑。见第 2.1 节。
>    **所以调超参的收益上限低于补多参考训练数据**（已提为 P0，见
>    [DATA_SCALING_PLAN.md](DATA_SCALING_PLAN.md)）。
>
> 本文剩余价值：全参侧的 LR / epochs 扫描设计与基线口径仍可用。
>
...
**评测基线**（[EVAL_RESULTS_20260815.md](EVAL_RESULTS_20260815.md)，`seed=0 / steps=40`）：
...
>
> **判定差异是否真实必须用配对检验**（`scripts/paired_eval_stats.py`）：六个模型跑同一批样本，
> 按样本作差看标准误。实测中「均值差 3.15 但 t=1.43 不显著」这类情况会出现——
> 单看均值会误判。
